...
| **Alliance convoy staging** | Multi-member convoys can depart from controlled regions only |
...
- **Convoy** — unlocks the Convoy Rig collector class, additional escort slots, faster transit
...
3. **Weekly tick timing across timezones.** Whichever hour is chosen advantages some regions. Possibly per-server tick times, set at server creation.
...
*All five core systems are now specced. Next: the screen inventory — a single map of every screen implied across these documents, and the data each one needs.*
