...
2. In **Window → General → Test Runner → EditMode**, run **Broodline.Frontier.Tests**. Eight tests cover outward mesh winding/finite geometry, the 10,000-triangle assembled ceiling and skin bindings, trait attachment/rest pose, simulation observation equivalence, formation validation/swaps, terminal breach feedback, and blink bones/reduced motion. These Unity tests are authored but **have not run here**. The existing repository reflection runner also includes the new assembly.
