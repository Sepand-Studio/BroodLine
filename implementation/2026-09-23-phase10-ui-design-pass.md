...
The Store pack ladder now has six distinct container silhouettes, from loose shards through pouch, case, crate and vault to a sealed reserve. Its purchase rows separate pack identity from price and the preview button, giving the latter a 44px minimum height. The Custom Chest shows a live count of selected rewards. The catalog now says **sample pulls**, which matches bible §8.3's rule against selling trait access, and the Season Pass displays that section's $9.99 price and four-week term. Its reward amounts by chest size and monetary saving still need catalog values before a truthful tier picker/value comparison can be shown; the UI does not invent those values. The new icons have editable sources at `implementation/art-source/store/` and a large/28px sheet at `implementation/reviews/phase10-store-pack-icons.png`.
