...
- Create: `client/Assets/Game/Editor.meta`, `client/Assets/Game/Editor/WaveSceneBuilder.cs` (+ `.meta`), `client/Assets/Game/Editor/Broodline.Game.Editor.asmdef` (+ `.meta`)
...
`client/Assets/Editor/BenchmarkBuilder.cs` already carries the iOS build path from Phase 0; follow it for this scene. Ensure **`UIFileSharingEnabled`** and **`LSSupportsOpeningDocumentsInPlace`** are `YES` in the generated `Info.plist`, or the Documents directory is not reachable.
...
**If `devicectl` cannot reach the container**, the fallback is the Xcode Devices window — Devices and Simulators → the device → the app → Download Container — and then copying `AppData/Documents/replay.bin` out of the `.xcappdata` bundle. Record which route worked; the next phase should not rediscover it.
